...
window.CONFIG = {
  ...
  fields: [
    ...
    { key: 'owner', label: '负责人', type: 'text', required: true, maxLength: 100 },
    { key: 'progressPercent', label: '项目进度（%）', type: 'number', min: 0, max: 100, step: 1, importOptional: true },
    // 历史字段不再显示或录入；保留兼容原 JSON / Excel，避免丢失已有文字。
    { key: 'tracking', label: '工作跟踪（历史保留）', aliases: ['工作跟踪'], type: 'textarea', maxLength: 4000, legacy: true, importOptional: true },
    { key: 'progress', label: '现阶段进展', type: 'textarea', maxLength: 4000 },
    { key: 'dueDate', label: '预计交付日期', type: 'date', required: true },
    { key: 'nextPlan', label: '下一步计划', type: 'textarea', requiredWhenOpen: true, maxLength: 4000 },
  ...
  limits: { maxFileBytes: 15 * 1024 * 1024, maxRecords: 5000, maxSnapshots: 100 },
  excel: { sheetName: '工作台账', idLabel: '唯一编号', moduleLabel: '模块', updatedLabel: '更新时间', statusLabel: '实际状态（只读）' }
};
