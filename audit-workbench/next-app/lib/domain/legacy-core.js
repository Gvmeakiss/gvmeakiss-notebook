import { enhancementFields, validateEnhancements, validateRecoveryKeys, retainHistory, combineHistory } from "./enhancements.js";
...
export function createCore(root) {
  "use strict";
  const C = root.CONFIG;
  const legacy = createV1Core({ CONFIG: V1_CONFIG, crypto: root.crypto });
  const modules = [...C.modules, ...(C.legacyModules || [])];
  const people = (owner) => [
    ...new Set(
      String(owner || "")
        .split(/[、,，;；\n]+/)
  ...
  const normalizePeople = (owner) => people(owner).join("、");
  const peopleKey = (owner) => JSON.stringify(people(owner).sort());
  const stageOverdue = (record, today = localDate()) =>
    !record.completed &&
    !!record.currentStageDueDate &&
    record.currentStageDueDate < today;
  ...
  const moduleName = (value) =>
    (modules.find((m) => m.id === value) || {}).label || value;
  ...
  function validateRecord(r, options = {}) {
    ...
    if (!modules.some((m) => m.id === r.module))
    ...
    if (
      ...
      (!people(r.owner).length || people(r.owner).some((n) => n.length > 100))
    )
      errors.push(
        "负责人／参与人员请填写至少一个姓名，单个姓名不超过100字，使用顿号分隔",
      );
    errors.push(...validateEnhancements(r, validDate, validTime));
  ...
  function validateBackup(data) {
    if (isObject(data) && data.schemaVersion === 1) {
      ...
      return {
        ...
        records: old.records,
        snapshots: old.snapshots.map((s) => ({ ...s, schemaVersion: 1 })),
      };
    }
    if (!isObject(data) || data.schemaVersion !== C.schemaVersion)
      throw new Error("备份格式或版本不兼容");
    const errors = validateRecords(data.records);
    const allowed = ["schemaVersion", "records", "snapshots", "exportedAt", "teamId"];
    if (data.teamId !== undefined && (typeof data.teamId !== "string" || !/^[A-Za-z0-9_-]{1,100}$/.test(data.teamId))) errors.push("团队编号无效");
    if (data.teamId && Array.isArray(data.records) && data.records.some(r => r.teamId && r.teamId !== data.teamId)) errors.push("台账混入其他团队记录");
    errors.push(...validateRecoveryKeys(Array.isArray(data.records) ? data.records : []));
    ...
    if (
      ...
      data.snapshots.forEach((s, i) => {
        ...
        if (
          ...
          ![1, C.schemaVersion].includes(s.schemaVersion)
        )
          errors.push(prefix + "数据版本无效");
        (s.schemaVersion === 1
          ? legacy.validateRecords(s.records, { snapshot: true })
          : validateRecords(s.records, { snapshot: true })
        ).forEach((e) => errors.push(prefix + e));
  ...
  function sameContent(a, b) {
    const ordered = value => Array.isArray(value) ? value.map(ordered) : value && typeof value === 'object'
      ? Object.fromEntries(Object.entries(value).filter(([,v])=>v!=='' && v!=null).sort(([x],[y])=>x.localeCompare(y)).map(([k,v])=>[k,ordered(v)])) : value;
    return ["module", ...C.fields.map((f) => f.key)].every((k) =>
      ["stageHistory", "recoveryEntries"].includes(k)
        ? JSON.stringify(ordered(a[k] || [])) === JSON.stringify(ordered(b[k] || []))
        : k === "progressPercent"
        ? percent(a) === percent(b)
        : k === "owner"
          ? peopleKey(a.owner) === peopleKey(b.owner)
          : (a[k] ?? "") === (b[k] ?? ""),
    );
  }
  const naturalKey = (r) =>
    JSON.stringify([r.module, r.name, peopleKey(r.owner)]);
  function prepareMerge(current, incoming) {
    ...
    return incoming.map((record) => {
      ...
      if (existing) {
        record = clone(record);
        for (const f of enhancementFields) if (!(f.key in record) && f.key in existing) record[f.key] = clone(existing[f.key]);
        // 归档状态由本工作区的归档/恢复操作维护；旧提交不能重新放回日常清单。
        record.archivedAt = existing.archivedAt || '';
        if (existing.teamId && record.teamId && existing.teamId !== record.teamId) throw new Error("不能合并不同团队的同编号记录");
        const history = combineHistory(existing.stageHistory, record.stageHistory);
        if (history.length) record.stageHistory = history;
      }
      return {
        record: clone(record),
  ...
  function applyMerge(current, plan, choices = {}) {
    ...
    plan.forEach((item) => {
      if (
        ...
        map.set(item.record.id, retainHistory(map.get(item.record.id), item.record, id));
    });
    const result = [...map.values()];
    const errors = [...validateRecords(result), ...validateRecoveryKeys(result)];
    if (errors.length) throw new Error(errors.join("\n"));
    return result;
  }
  ...
  function filtered(records, filters, frozen = false) {
    ...
    return records.filter(
      (r) =>
        ...
        (!filters.owner || people(r.owner).includes(filters.owner)) &&
        (!filters.status ||
